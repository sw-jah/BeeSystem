...
public class EventDetailFrame extends JFrame {
    ...
    private void initUI() {
        ...
        JLabel statusLabel = new JLabel(eventData.status);
        ...
        statusLabel.setBackground("신청마감".equals(eventData.status) || "종료".equals(eventData.status) ? ORANGE_CLOSED : GREEN_PROGRESS);
        ...
        if ("진행중".equals(eventData.status) && eventData.currentCount < eventData.totalCount) {
            ...
            applyButton.addActionListener(e -> {
                ...
                // 1. 학과 체크
                if (!eventData.targetDept.equals("총학생회") && !eventData.targetDept.equals(userDept)) {
                    showSimplePopup("신청 불가", "본 행사는 '" + eventData.targetDept + "'\n학생만 참여 가능합니다.");
                    return;
                }
